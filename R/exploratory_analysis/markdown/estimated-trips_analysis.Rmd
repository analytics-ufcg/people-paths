...
```{r}
estimated_trips_result %>% 
  filter(dist_median < 6000 & duration_median >= 30) %>% 
  group_by(route) %>% 
  summarise(quantity = sum(quantity_trips)) 

outlier <- estimated_trips_result %>% 
  filter(route == "022") %>%
  filter(dist_median < 6000 & duration_median >= 30)
...
estimated_trips_result %>% 
  filter(route == "022") %>%
  summarise(duracao = median(duration_median))
```

  Os outliers são aquelas viagens que apresentam duração mediana acima de 25 minutos. Nessa análise, restringimos as viagens para aquelas com duração mediana igual ou maior que 30, onde vemos que a rota 022 possui o maior número de viagens, com cerca de 284. As rotas 370 e 532 também se destacam, com 162 e 129 viagens feitas, respectivamente. 
  ...
  Analisando a rota 022 mais profundamente.
```{r}
outlier %>%
  group_by(start_hour) %>% 
  summarise(quantity = sum(quantity_trips)) %>% 
  ggplot(aes(x = start_hour, y = quantity)) + geom_col() + theme_bw() + labs(x = "hora", y = "quantidade de viagens")
```
...
outlier %>%
  ...
  ggplot(aes(x = start_hour, y = dist)) + geom_col() + theme_bw() + labs(x = "hora", y = "distância percorrida (metro)")
```
...
outlier %>%
  ...
  ggplot(aes(x = start_hour, y = duration)) + geom_col() + theme_bw() + labs(x = "hora", y = "duração da viagam")
```
...
estimated_trips_result %>%
  group_by(route) %>%
  summarise(qntt = sum(quantity_trips), median_dist = median(dist_median)) %>% 
  filter(qntt > 100) %>%
  top_n(10) %>%
  ggplot(aes(x = reorder(route, -median_dist), y = median_dist)) + labs(x = "rota", y = "distância") + geom_col() + theme_classic()
```
...
estimated_trips_result %>% 
  ...
  filter(quantity > 300) %>% 
  top_n(10) %>% 
  ggplot(aes(x = reorder(route, -quantity), y = quantity/1000)) + labs(x = "rota", y = "quantidade de viagens (em milhões)") + geom_bar(stat = "identity") + theme_classic()
```
...
estimated_trips_result %>%
  group_by(route) %>% 
  summarise(qntt = sum(quantity_trips), duration = median(duration_median)) %>%
  filter(qntt > 300) %>% 
  top_n(10) %>% 
...
estimated_trips_result %>%
  ...
  top_n(-10) %>% 
  ggplot(aes(x = reorder(route, duration), y = duration)) +
    labs(x = "rota", y = "duração da viagem na mediana (em minutos)") +
    geom_bar(stat = "identity") +
    theme_classic()
```
