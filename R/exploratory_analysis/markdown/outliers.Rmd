...
```{r}
estimated_trips_result %>%
  filter(dist_median <= 6000 & duration_median > 50 & week_day == "5") %>% 
  group_by(route) %>%
  summarise(day_outliers = n()) %>% 
  filter(day_outliers > 40) %>% 
  ggplot(aes(x = route, y = day_outliers)) + geom_col() + theme_bw() + labs(x = "Rota", y = "Quantidade de viagens")
...
```{r}
estimated_trips_result %>%
  filter(dist_median <= 6000 & duration_median > 50 & route == "370") %>% 
