...
dataPerc <- estimated_trips_result %>%
  group_by(route) %>% 
  summarise(noventa_percentil = quantile(duration_median, probs = 0.90), duração_mediana_geral = median(duration_median), qntt = sum(quantity_trips)) %>% 
  filter(qntt > 300) %>% 
  top_n(10)

dataPercLong <- gather(dataPerc, key="measure", value="value", c("noventa_percentil", "duração_mediana_geral"))
