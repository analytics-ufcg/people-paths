...
```{r}
"/home/admin/ivan/data/od-matrix/full/output/result.csv"
...
estimated_trips_result %>%
  ...
  filter(qntt < 300) %>% 
  ggplot(aes(x = reorder(route, qntt), y = qntt)) + labs(x = "route") + geom_col() + theme_classic()
...
days <- estimated_trips_relevant_routes %>%
  group_by(route, week_day) %>% 
...
```{r}
hour_group <- data.frame(from_hour = c(4:23),
...
```
  ...
  Como podemos observar, pela manhã a rota 303 ultrapassa em quantidade de viagens a rota 203. A noite e a tarde a 203 volta a exercer robusta liderança. Observando também as rotas 203, 503, 602, 023 e 502 vemos que suas viagens são mais populares a noite. Por outro lado, as rotas 303, 022, 040, 030, 603, 507, 204 e as que se seguem são mais populares pela manhã, que é a faixa de horário mais popular durante todo o dia. 
  Não vemos nenhuma rota sendo mais popular no horário da tarde, ou seja, aquela que se encontra entre as 11h e as 13h. Um destaque evidente fica por conta da rota 503, que dispara no horário da noite, ultrapassando a rota 303 e se aproximando da 203. A 503 é uma rota predominantemente noturna.

Vamos ver agora a distribuição da quantidade de viagens nos três principais horários do dia: 6-8, 11-13, 17-19.   
```{r}
estimated_trips_result <- estimated_trips_result %>% 
  mutate(from_hour = lubridate::hour(lubridate::hms(from_time)))

hours <- estimated_trips_result %>% 
  group_by(route, from_hour) %>% 
  summarise(qntt = sum(quantity_trips)) %>% 
  merge(hour_group) %>% 
  group_by(hour_group) %>% 
...
hours$hour_group <- factor(hours$hour_group, levels = c("Manhã", "Tarde", "Noite"))


ggplot(hours, aes(x = hour_group, y = qntt)) + geom_col(position="dodge", stat='identity') + labs(x = "Horário de pico", y = "Quantidade de viagens")
ggsave(file="picos.pdf")
```
...
```
  ...
  Apesar das 11h as 13h ser o horário com o menor número de viagens no geral entre os três principais horários, rotas como 506, 646 e 617 apresentam suas viagens como mais longas na mediana nesse horário. A rota 305, curiosamente, apresenta a mesma mediana de distância nos três horários.
  Para o horário da manhã, aquele que concentra a maior quantidade de viagens, são apenas duas as rotas que aprensentam suas viagens mais longas nesse horário, a 659, 508 e 684.  
...
estimated_trips_result %>%
  group_by(route) %>% 
  summarise(dur = median(duration_median), qntt = sum(quantity_trips)) %>% 
  filter(qntt > 300) %>% 
  top_n(20) %>% 
  ggplot(aes(x = reorder(route, -dur), y = dur)) + geom_col(stat='identity', colour = "gray") +
  labs(x = "route", y = "ninety percentille - duration")
```
...
dataPerc <- estimated_trips_result %>%
  group_by(route) %>% 
  summarise(duração_noventa_percentil = quantile(duration_median, probs = 0.90), duração_mediana_geral = median(duration_median), qntt = sum(quantity_trips)) %>% 
  filter(qntt > 300) %>% 
  top_n(10)

dataPercLong <- gather(dataPerc, key="measure", value="value", c("duração_noventa_percentil", "duração_mediana_geral"))

ggplot(dataPercLong, aes(fill = measure, x = reorder(route, -value), y = value)) + geom_col(position="dodge", stat='identity') + labs(x = "rota", y = "duração da viagem")
```

  Medimos o 90 e o 95 percentil do tempo das viagens pelas rotas. Como podemos observar, a rota 022 varia bastante em seus tempos de viagens, seu 95 percentil mostra que 95% de suas viagens estão inclusas no intervalo de 0 a aproximadamente 50 minutos. 
  O 90 percentil, por sua vez, nos mostra que as rotas não variam significativamente em seus tempos de viagens, estando inclusas no intervalo de 0 a pouco mais de 20 minutos.
...
9) DISTRIBUIÇÃO DAS VIAGENS NAS HORAS DO DIA
```{r}
estimated_trips_result <- estimated_trips_result %>% 
  mutate(from_hour = lubridate::hour(lubridate::hms(from_time)))
...
estimated_trips_result %>% 
  dplyr::group_by(from_hour) %>% 
  dplyr::summarise(quantity = sum(quantity_trips)) %>% 
  ggplot(aes(x = from_hour, y = quantity)) + labs(x = "Horário do dia", y = "Quantidade de viagens") + geom_col() + theme_classic()
ggsave(file="horario_dia.pdf", width = 24.88, height = 16.88, units = "cm")
```
