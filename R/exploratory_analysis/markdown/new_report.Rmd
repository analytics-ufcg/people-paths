...
ggplot(hours, aes(x = hour_group, y = qntt)) + geom_col(position="dodge", stat='identity') + labs(x = "período do dia", y = "quantidade de viagens")
```
...
5) Intervalo de confiança - gráficos de barra
...
```
```{r}
ic_estimated_trips_result <- summarySE(dat2, measurevar="dist", groupvars=c("route", "hour_group"))
...
ggplot(ic_estimated_trips_result, aes(x= reorder(route,-dist), y=dist, fill=hour_group)) + 
    geom_bar(position=position_dodge(), stat="identity") +
    geom_errorbar(aes(ymin=dist-se, ymax=dist+se),
                  width=.2,                    # Width of the error bars
                  position=position_dodge(.9)) + labs(x = "rota", y = "distância percorrida", fill="Horário")
```

QUANTIDADE DE VIAGENS PELAS HORAS DO DIA
```{r}
estimated_trips_result %>% 
  group_by(start_hour) %>% 
