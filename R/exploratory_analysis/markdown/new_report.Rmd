...
ic_estimated_trips_result <- summarySE(hours, measurevar="qntt", groupvars=c("route"))

ic_estimated_trips_result

ggplot(ic_estimated_trips_result, aes(x= reorder(route,-qntt), y=qntt)) + 
...
hours <- estimated_trips_result %>% 
  group_by(route, start_hour) %>% 
  filter(route == "203" || route == "303" || route == "503" || route == "602" || route == "023" || route == "040" || route == "603" || route == "507" || route == "022") %>% 
  summarise(qnt = sum(quantity_trips))
  merge(hour_group) %>% 
  group_by(hour_group, route) %>% 
  summarise(qntt = median(qnt))
