...
antigo_data <- read.csv("/local/juninho/aggregate_output/2017_05_02_full_agg", stringsAsFactors=F)
```
...
embarque <- mapdata_emb %>% 
  ...
  ggplot() + geom_polygon(data=shpf, aes(x=long, y=lat, group=group))+ 
  geom_hex(data=embarque, aes(x=xEmb, y=yEmb), bins=40) + labs(x = "longitude", y = "latitude") + scale_fill_continuous(low = "#132B43",
  high = "#56B1F7",
  space = "Lab",
  na.value = "grey50",
  guide = "colourbar",
...
embarque %>% 
  ggplot() +  
      ...
      theme(axis.ticks.y = element_blank(),axis.text.y = element_blank(), # get rid of x ticks/text
            axis.ticks.x = element_blank(),axis.text.x = element_blank(), # get rid of y ticks/text
            plot.title = element_text(lineheight=.8, face="bold", vjust=1))+ # make title bold and add space
      geom_density2d(aes(x=xEmb, y=yEmb), data=embarque, alpha=0.4, size=3, color="grey20")+# to get outline
      geom_density2d(aes(x=xEmb, y=yEmb), data=embarque, alpha=0.4, size=2)+
      scale_colour_gradientn("Building\narea (sq-km)", 
          colours=c( "#f9f3c2","#660000"))+ # change color scale
      coord_equal(ratio=0.7) # square plot to avoid the distortion
```
...
desembarque <- mapdata %>% 
  ...
  ggplot() + geom_polygon(data=shpf, aes(x=long, y=lat, group=group))+ 
    geom_density2d(data=desembarque, aes(x=x, y=y), color="red") + labs(x = "longitude", y = "latitude")
```

Segundo plot do DESEMBARQUE
```{r}
desembarque %>% 
  ggplot() +  
      geom_polygon(data=shpf, aes(x=long, y=lat, group=group), fill="grey40", 
          colour="grey90", alpha=1)+
      labs(x="", y="", title="Desembarque por bairros")+ #labels
      theme(axis.ticks.y = element_blank(),axis.text.y = element_blank(), # get rid of x ticks/text
            axis.ticks.x = element_blank(),axis.text.x = element_blank(), # get rid of y ticks/text
            plot.title = element_text(lineheight=.8, face="bold", vjust=1))+ # make title bold and add space
      geom_density2d(aes(x=x, y=y), data=desembarque, alpha=0.4, size=3, color="grey20")+# to get outline
      geom_density2d(aes(x=x, y=y), data=desembarque, alpha=0.4, size=2)+
      scale_colour_gradientn("Building\narea (sq-km)", 
          colours=c( "#f9f3c2","#660000"))+ # change color scale
      coord_equal(ratio=0.8) # square plot to avoid the distortion
```

Density map - DESEMBARQUE

```{r}
ggplot() + 
  geom_polygon(data=shpf, aes(x=long,y=lat,group=group), fill="black") +
  stat_bin2d(data=desembarque, aes(x=x,y=y), bins=30, colour="white") +
  ...
  guide = "colourbar",
  aesthetics = "color") + 
  geom_path(data=shpf, aes(x=long,y=lat,group=id), colour='white') +
  coord_equal() +
  theme_void() +
  coord_equal(ratio=0.6) + # square plot to avoid the distortion
  labs(fill="Quantidade de viagens")
ggsave(file="desembNoite.pdf", width = 24.88, height = 16.88, units = "cm")
```

Density map - EMBARQUE
```{r}
ggplot() + 
  geom_polygon(data=shpf, aes(x=long,y=lat,group=group), fill="black") +
  stat_bin2d(data=embarque, aes(x=xEmb,y=yEmb), bins=30, colour="white") +
  scale_fill_gradient(low = "#132B43",
  high = "#56B1F7",
  space = "Lab",
  na.value = "grey50",
  ...
  coord_equal(ratio=0.6) + # square plot to avoid the distortion
  labs(fill="Quantidade de viagens") 
  ggsave(file="embarqueNoite.pdf", width = 20.88, height = 12.88, units = "cm")
```