...
ant_data <- read.csv("/local/juninho/aggregate_output/2017_05_09_full_agg", stringsAsFactors=F)
```
...
```{r}
rotas <- tdata %>% 
  group_by(busCode, route, from_hour, end_hour) %>% 
  dplyr::summarise(bus_trips = n()) %>% 
  filter(route == "303" & (from_hour >= 6 & end_hour <= 8))

rotas

rotas303 <- rotas %>% 
  group_by(busCode, bus_trips) %>% 
  dplyr::summarise(trips = sum(bus_trips)) 
